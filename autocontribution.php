...
function autocontribution_civicrm_postCommit(string $op, string $objectName, int $objectId, &$objectRef): void{
  if ($objectName == 'Activity' && $op == 'edit'){    
    ...
    if ($objectRef->activity_type_id == $activityID){
      if ($objectRef->status_id == 2){
...
function autocontribution_civicrm_post(string $op, string $objectName, int $objectId, &$objectRef) : void{
  if ($objectName == 'CustomField' && $op == 'delete'){
    //CHECKS IF FIELD EDITED WAS EXTENDED TO CONTRIBUTION
    try{
      $results = civicrm_api4('CustomField', 'delete', [
        'where' => [
          ['name', '=', "autocon_cloned_".$objectId],
        ],
        'checkPermissions' => TRUE,
      ]);
    } catch (CRM_Core_Exception $err){
